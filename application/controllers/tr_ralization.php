<?php
	if ( ! defined('BASEPATH')) exit('No direct script access allowed');
			...
			$this->access->set_module('tr_ralization.tr_ralization');
			$this->access->user_page();
			}
			function index(){
			$this->render->add_view('app/tr_ralization/list');
			$this->render->build('Realisasi Plan');
			$this->render->show('Realisasi Plan');
		}
		function table_controller(){
			$data = $this->tr_ralization_model->list_controller();
			send_json($data);
		}
		...
		function form($registration_id = 0)
		{
			...
			$result = $this->tr_ralization_model->read_id($registration_id);
			if($result){
				$data = $result;
				...
				$data['tr_ralization_plain_first_date'] = format_new_date($result['tr_ralization_plain_first_date']);
				$data['tr_ralization_plain_last_date'] = $result['tr_ralization_plain_last_date'];
				$data['tr_ralization_actual_date'] = $result['tr_ralization_actual_date'];
				$data['tr_ralization_target_date'] = $result['tr_ralization_target_date'];
			...
			// List sparepart
			$this->render->add_view('app/tr_ralization/transient_list_sparepart', $data);
			$this->render->build('Data Sparepart');
			
			// list panel asuransi
			$this->render->add_view('app/tr_ralization/transient_list_panel', $data);
			$this->render->build('Data Panel Asuransi');

			// list jasa
			$this->render->add_view('app/tr_ralization/transient_list', $data);
			$this->render->build('Data Jasa');
			// list bahan
			$this->render->add_view('app/tr_ralization/transient_list_bahan', $data);
			$this->render->build('Data Bahan');
			// list cat
			$this->render->add_view('app/tr_ralization/transient_list_cat', $data);
			$this->render->build('Data Cat');
			

			// list foto
			$this->render->add_view('app/tr_ralization/transient_list_foto');
		...
		function form_action($is_delete = 0) // jika 0, berarti insert atau update, bila 1 berarti delete
		{
			...
			{
				$this->load->model('tr_ralization_model');
				$id = $this->input->post('i_tr_ralization_id');
				$is_process_error = $this->tr_ralization_model->delete($id);
		...
		// cek data berdasarkan kriteria
			if ($this->form_validation->run() == FALSE) send_json_validate();
				...
				$tr_ralization_id = $this->input->post('i_tr_ralization_id');

				$data['registration_id'] = $this->input->post('row_id');
				$data['employee_group_id'] = $this->input->post('i_employee_group_id');
				$data['tr_ralization_plain_first_date'] = $this->input->post('i_first_date');
				$data['tr_ralization_plain_last_date'] = $this->input->post('i_last_date');
				$data['tr_ralization_actual_date'] = $this->input->post('i_actual_date');
				$data['tr_ralization_target_date'] = $this->input->post('i_target_date');
				$data['employee_group_id2'] = $this->input->post('i_employee_group_id2');
				$data['tr_ralization_gabungan_lain'] = $this->input->post('i_gabungan_lain');
				$data['tr_ralization_las_lain'] = $this->input->post('i_las_lain');
				
				// simpan transient jasa
				$list_tr_ralization_detail_date		= $this->input->post('transient_tr_ralization_detail_date');
				...
				$list_tr_ralization_detail_progress	= $this->input->post('transient_tr_ralization_detail_progress');

			if(!$list_tr_ralization_detail_date) send_json_error('Simpan gagal. Data Realisasi Plan masih kosong');
				$total_price = 0;
				$total_progress = 0;
				$jumlah_jasa = 0;
				$items = array();
				...
				// jasa
				if($list_tr_ralization_detail_date){
					foreach($list_tr_ralization_detail_date as $key => $value)
					{
				//$get_purchase_price = $this->registration_model->get_purchase_price($list_product_id[$key]);
						$items[] = array(
							'tr_ralization_detail_date' => ($list_tr_ralization_detail_date[$key]),
							'tr_ralization_detail_progress' => $list_tr_ralization_detail_progress[$key],
						...
						$total_progress += $list_tr_ralization_detail_progress[$key];
						$jumlah_jasa++;
					}
				}
				...
				if($list_bahan_name){
					foreach($list_bahan_name as $key => $value)
					{
						...
						if($check > 1){
							...
							$get_data_materails = $this->tr_ralization_model->get_data_material($tm_stock_id_original);
				...
				$data['tr_ralization_material_total'] = $total_material;
				$data['tr_ralization_progress'] = $total_progress / $jumlah_jasa;
				$data['tr_ralization_total'] = $total_price;

				if(empty($tr_ralization_id)) // jika tidak ada id maka create
				{
			//$data['registration_code'] = format_code('registrations','registration_code','PU',7);
				//$error = $this->tr_ralization_model->create($data, $items, $items_material, $items_foto,$items_sparepats);
				send_json_action($error, "Data telah ditambah", "Data gagal ditambah");
				}
				else // id disebutkan, lakukan proses UPDATE
				{
					//$error = $this->tr_ralization_model->update($tr_ralization_id, $data, $items,$items_material, $items_foto,$items_sparepats);
			...
			function detail_list_loader($registration_id=0)
			{
				...
				$data = $this->tr_ralization_model->detail_list_loader($registration_id);
				$sort_id = 0;

				foreach($data as $key => $value)
				{
				$data[$key] = array(
					form_transient_pair('transient_tr_ralization_detail_date', format_new_date($value['tr_ralization_detail_date']), $value['tr_ralization_detail_date'],
							array(
									'transient_tr_ralization_detail_id' => $value['tr_ralization_detail_id'],
						...
						form_transient_pair('transient_tr_ralization_detail_progress', $value['tr_ralization_detail_progress'])
					);
				}
			...
			function detail_form($registration = 0) // jika id tidak diisi maka dianggap create, else dianggap edit
			{
				...
				if (strlen(trim($index)) == 0) {
					...
					$data['index'] = '';
					$data['tr_ralization_detail_id'] = '';
					$data['tr_ralization_detail_date'] = '';
					$data['workshop_service_id'] = '';
					$data['workshop_service_price'] = '';
					$data['workshop_service_name'] = '';
					$data['workshop_service_job_price'] = '';
					$data['tr_ralization_detail_progress'] = '';
				} else {
					$data['index'] = $index;
					//$data['workshop_service_name'] = array_shift($this->input->post('transient_workshop_service_name'));
					$data['tr_ralization_detail_id'] = array_shift($this->input->post('transient_tr_ralization_detail_id'));
					$data['tr_ralization_detail_date'] = array_shift($this->input->post('transient_tr_ralization_detail_date'));
					...
					$data['tr_ralization_detail_progress'] = array_shift($this->input->post('transient_tr_ralization_detail_progress'));
					
				}
				$data['progress']=array('25'=>'25%','50'=>'50%','70'=>'70%','90'=>'90%','100'=>'100%');
				
					$this->load->helper('form');
					$this->render->add_form('app/tr_ralization/transient_form', $data);
			...
			function detail_form_action()
			{
				...
				$this->form_validation->set_rules('i_tr_ralization_detail_date', 'Tanggal', 'trim|required|valid_date|sql_date');
				$this->form_validation->set_rules('i_workshop_service_id', 'Jasa', 'trim|required');
				$this->form_validation->set_rules('i_tr_ralization_detail_progress', 'Progress', 'trim|required|max_value[100]');
				$index = $this->input->post('i_index');
			// cek data berdasarkan kriteria
			if ($this->form_validation->run() == FALSE) send_json_validate();
				$no = $this->input->post('i_index');
				
				$tr_ralization_detail_id = $this->input->post('i_tr_ralization_detail_id');
				$tr_ralization_detail_date = ($this->input->post('i_tr_ralization_detail_date'));
				...
				$tr_ralization_detail_progress	= $this->input->post('i_tr_ralization_detail_progress');
				//send_json_error($no);
				$data = array(
				form_transient_pair('transient_tr_ralization_detail_date', format_new_date($tr_ralization_detail_date), $tr_ralization_detail_date,
							array(
									'transient_tr_ralization_detail_id' => $tr_ralization_detail_id)),
						...
						form_transient_pair('transient_tr_ralization_detail_progress', $tr_ralization_detail_progress)
					);
	...
	function detail_list_loader_sparepart($registration_id=0)
	{
		...
		$data = $this->tr_ralization_model->detail_list_loader_sparepart($registration_id);
		$sort_id = 0;
		foreach($data as $key => $value) 
		{	
	...
	function detail_list_loader_panel($row_id=0)
			{
				...
				$data = $this->tr_ralization_model->detail_list_loader_panel($row_id);
				$sort_id = 0;
				foreach($data as $key => $value) 
				{
				$data[$key] = array(
	...
	function detail_list_loader_bahan($registration_id=0)
	{
		...
		$data = $this->tr_ralization_model->detail_list_loader_bahan($registration_id);
		$sort_id = 0;
		foreach($data as $key => $value) 
		{	
	...
	{		
		...
		$this->render->add_form('app/tr_ralization/transient_form_bahan', $data);
		$this->render->show_buffer();
	}
	...
	function detail_list_loader_cat($registration_id=0)
	{
		...
		$data = $this->tr_ralization_model->detail_list_loader_cat($registration_id);
		$sort_id = 0;
		foreach($data as $key => $value) 
		{	
	...
	function detail_list_loader_foto($registration_id=0)
	{
		...
		$data = $this->tr_ralization_model->detail_list_loader_foto($registration_id);
	...
	{		
		...
		$this->render->add_form('app/tr_ralization/transient_form_sparepart', $data);
		$this->render->show_buffer();
	}
	...
	{		
		...
		$this->render->add_form('app/tr_ralization/transient_form_cat', $data);
		$this->render->show_buffer();
	}
	...
	{		
		...
		$this->load->helper('form');
		$this->render->add_form('app/tr_ralization/transient_form_foto', $data);
	...
	{		
		...
		$photo_type_name = $this->tr_ralization_model->get_photo_type_name($photo_type_id);
	...
	function load_workshop_service()
	{
		...
		$query = $this->tr_ralization_model->load_workshop_service($id);
		$data = array();
		...
		function load_detail_material()
	{
		...
		$query = $this->tr_ralization_model->load_detail_material($id);
		$data = array();
