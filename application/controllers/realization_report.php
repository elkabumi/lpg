...
if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	class Realization_report extends CI_Controller{
		...
		function index(){
				...
				$this->render->add_form('app/realization_report/form', $data);
				$this->render->build('Realisasi Plan');
				
				$this->render->add_view('app/realization_report/transient_list_kulak');
				$this->render->build('Detail Plan');
			
				$this->render->show('Realisasi Plan');
		}	
	function form($id){
			$result = $this->realization_report_model->read_id($id);
			if($result){
				$data = $result;
				$data['row_id'] = $id;
		...
		$this->load->helper('form');
		$this->render->add_form('app/realization_report/form_ralization', $data);
		$this->render->build('Realisasi Plan');
		$this->render->show('Realisasi Plan');
	}
	...
	function form_action($is_delete = 0){
		...
		if($is_delete){
			$is_proses_error = $this->realization_report_model->delete($id);
		...
		if(empty($id)){
			$error = $this->realization_report_model->create($data,$items_plan_detail);
			send_json_action($error, "Data telah ditambah", "Data gagal ditambah");
		}else{
		
			$error = $this->realization_report_model->update($id,$data);
		...
		function detail_table_loader_kulak($date = 0) {
			if($date == 0){
				...
				$data = $this->realization_report_model->detail_table_loader_kulak($date);
				...
				$link = "<a href=".site_url('realization_report/form/'.$value['tr_plan_detail_id'])." class='link_input'> Realisasi </a>";
	...
	function load_data_truck()
	{
		...
		$query = $this->realization_report_model->load_data_truck($id);
		$data = array();
	...
	function load_data_spbe()
	{
		...
		$query = $this->realization_report_model->load_data_spbe($id);
		$data = array();
	...
	function load_data_route()
	{
		...
		$query = $this->realization_report_model->load_data_route($id);
		$data = array();
	...
	function report($i_date_1){
		
		$date1 = explode("-", $i_date_1);

			$data['detail'] = $this->realization_report_model->report($i_date_1);
